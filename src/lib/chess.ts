...
/**
 * NEW: Helper to determine the PGN result string (1-0, 0-1, 1/2-1/2, *)
 * This is used when constructing the PGN headers before saving.
 */
export const getGameResult = (game: Chess): string => {
  if (!game.isGameOver()) {
  ...
  // Default to ongoing if state is ambiguous or not fully finished
  return '*';
};
...
export const getGameFromPgn = (pgn: string): Chess => {
  ...
  try {
    ...
    console.error("Failed to load PGN:", e, pgn);
  }
  return game;
};
...
export const setGameHeaders = (
  game: Chess,
  params: { white?: Player; black?: Player; resigned?: Color; site?: string; event?: string } = {}
): Chess => {
  game.setHeader("Event", params.event || "Tembo Game");
  game.setHeader("Site", params.site || "Tembochess");
  game.setHeader(
    "Date",
