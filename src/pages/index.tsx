...
export default function GameAnalysis() {
  ...
  const { pgn, gameType, gameId } = router.query;

  const gameEval = useAtomValue(gameEvalAtom);
  const game = useAtomValue(gameAtom);
  const board = useAtomValue(boardAtom);
  const { setPgn } = useChessActions(gameAtom); // Use gameAtom from analysis states

  const showMovesTab = game.history().length > 0 || board.history().length > 0;

  // Load game from PGN when query parameters are present
  useEffect(() => {
    if (pgn && typeof pgn === 'string') {
      try {
        ...
        // Clear the query parameters from URL after loading to prevent reload issues
  ...
  return (
    <Grid container gap={4} justifyContent="space-evenly" alignItems="start">
      <PageTitle title="Tembo Game Analysis" />
